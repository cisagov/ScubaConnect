...
`create_output_storage` (bool) [default=True]::: If true, will create a local storage container and write results there. 
Results can be written to additional buckets via extra_output_container_urls
`extra_output_container_urls` (list(string)) [default=[]]::: Extra output container URLs to write results to. URL format: https://<account>.blob.core.windows.net/<container>    
...
`extra_output_container_sas_tokens` (list(string)) [default=[]]::: Optional SAS tokens for extra output containers. Must match the order of extra_output_container_urls.
