...
Required::
`contact_email` (string) ::: Email to notify for alerts and before certificate expiry
...
A log-based alert is pre-configured to send an email to `contact_email` when this occurs.
