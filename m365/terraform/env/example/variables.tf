...
variable "app_name" {
  ...
  validation {
    ...
    error_message = "App name cannot exceed 17 characters due to resource name limits. Either reduce length or set `prefix_override` to a shorter string"
  }
}
...
variable "prefix_override" {
  ...
  description = "Prefix for resource names. If null, one will be generated from app_name"
    validation {
    condition     = var.prefix_override == null || (length(var.prefix_override) <= 17 && length(var.prefix_override) > 1)
