...
resource "azurerm_storage_account" "storage" {
  count               = var.create_output_storage || var.input_storage_container_url == null ? 1 : 0
...
resource "azurerm_storage_container" "output" {
  count                 = var.create_output_storage ? 1 : 0
...
locals {
  ...
  output_storage_container_urls = concat(
    var.create_output_storage ? ["${azurerm_storage_account.storage[0].primary_blob_endpoint}${azurerm_storage_container.output[0].name}"] : [],
    var.extra_output_container_urls
  )
  
  output_storage_container_sas_tokens = concat(
    var.create_output_storage ? [""] : [],
    length(var.extra_output_container_sas_tokens) > 0 ? var.extra_output_container_sas_tokens : [for _ in var.extra_output_container_urls : ""]
  )
}
